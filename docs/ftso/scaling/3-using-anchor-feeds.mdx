...
## Query DA-layer for price and proof
...
For example, to get the prices for `BTC/USD` query the `anchor-feeds-with-proof` POST endpoint as follows:
...
The following example demonstrates how to query the price from a DA-layer and provide the price and proof to the smart contract.
...
## Query scaling data provider directly

If you run a data provider, you can use the data provider API to get the price and proof for the feeds you are interested in.
The on chain part is the same as in the previous example, but the offchain query is different.

```ts
async function getDataProviderData(feedId: string, voting_round_id: number) {
    return await (await fetch(DATA_PROVIDER_URL + `specific-feed/${feedIds}/${voting_round_id}`,
        {
            method: "GET",
            headers: { "X-API-KEY": API_KEY as string, "Content-Type": "application/json" },
        })).json();
